...
export function FaqSection() {
  ...
  return (
    <section className="flex flex-col items-center justify-center py-24 px-4 md:px-8 bg-muted/30 border-y border-border">
      <SectionTitle
        eyebrow="FAQ"
      ...
      <div className="mt-12 w-full max-w-2xl space-y-1">
        {faqs.map((faq, i) => (
          <div key={i} className="border border-border bg-card overflow-hidden">
            ...
            {open === i && (
              <div className="border-t border-dashed border-border px-5 py-4 text-sm/6 text-muted-foreground">
                {faq.a}
              </div>
            )}
          </div>
